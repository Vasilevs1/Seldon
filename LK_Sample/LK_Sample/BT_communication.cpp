...
bt_out::bt_out(string a) {
	...
	if (h_com != INVALID_HANDLE_VALUE)
		...
		printf("bluetooth handle invalid\n");
}
bt_out::~bt_out()
{
...
void bt_out::bt_send(const unsigned char* bytes_to_send, int size) {
	...
	{
		bluetooth_buffer_[i + 6] = bytes_to_send[i];
	}
	if (h_com == INVALID_HANDLE_VALUE || !valid)
	{
